...
const uint8_t DEVICE_ID = 2; // 1 for master, 2+ for slaves
...
// Web server functions (Master only)
void initWebServer();
void handleRoot();
void handleData();
void handleNotFound();
...
void setup() {
  ...
  // Initialize ESP-NOW (for both master and slave)
  initESPNow();
}
...
void initESPNow() {
  ...
  if (!IS_MASTER) {
    ...
    // For master, we already connected to an AP, so keep STA_AP mode
...
void updateDisplay() {
  ...
  for (int i = 0; i < MAX_PEERS && shownDevices < 3; i++) {
    if (deviceActive[i] && i != deviceIndex) {
      displayPtr->print("ID ");
      displayPtr->print(deviceReadings[i].sender_id);
      displayPtr->print(": ");
